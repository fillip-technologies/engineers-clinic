...
@section('content')
    ...
    <section class="rounded-2xl border border-gray-200 bg-gradient-to-r from-indigo-50 via-purple-50 to-blue-50 px-6 py-6 sm:px-8">
        ...
        <h1 class="mt-3 text-3xl font-semibold tracking-tight text-gray-900 sm:text-4xl">{{ $college->college_name ?? 'College' }}</h1>
    ...
    <section class="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 hover:shadow-md transition">
            ...
            <p class="mt-3 text-3xl font-semibold text-gray-900">{{ $activeInternships }}</p>
        </div>

        <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 hover:shadow-md transition">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Completed</p>
            <p class="mt-3 text-3xl font-semibold text-gray-900">{{ $completed }}</p>
        </div>
    </section>
                ...
                <tbody class="divide-y">
                    @forelse ($students as $student)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-4 font-medium text-gray-900">{{ $student->user->name ?? 'N/A' }}</td>
                            <td class="px-5 py-4 text-gray-600">{{ $student->user->email ?? 'N/A' }}</td>
                            <td class="px-5 py-4 text-gray-600">{{ $student->course_name ?? 'N/A' }}</td>

                            <td class="px-5 py-4">
                                @php
                                    $progress = $student->enrollments()->first()->progress ?? rand(30, 100);
                                @endphp
                            ...
                            <td class="px-5 py-4">
                                @php
                                    $status = $student->enrollments()->first()->status ?? 'Active';
                                @endphp
